...
import type { Time as _Time, UserDataEntry as _UserDataEntry, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async deleteData(): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.deleteData();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async getLastUpdated(): Promise<Time | null> {
        if (this.processError) {
            try {
    ...
    async getUserData(): Promise<UserDataEntry | null> {
        if (this.processError) {
            try {
    ...
    async saveUserData(arg0: string): Promise<Time> {
        if (this.processError) {
            try {
...
function from_candid_opt_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_UserDataEntry]): UserDataEntry | null {
    return value.length === 0 ? null : value[0];
}
